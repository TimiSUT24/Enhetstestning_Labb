...
namespace UnitTesting
{
    public class LibrarySystem
    {
        ...
        {
            if(book.ISBN == null || book.ISBN.Length != 13 || book.ISBN.Any(char.IsLetter))
            {
        ...
        public bool RemoveBook(string isbn)
        {
            ...
            if(book.IsBorrowed == true || book == null)
            {
                return false;
            }                    
            if (book != null)
        ...
        {
            return books.FirstOrDefault(b => b.ISBN != null && b.ISBN == isbn);
        }

        public List<Book> SearchByISBNPartial(string isbn)
        {
            return books.Where(b => b.ISBN != null && b.ISBN.Contains(isbn, StringComparison.OrdinalIgnoreCase)).ToList();
        }
        ...
        public decimal CalculateLateFee(string isbn, int loanPeriodDays)
        {
            int daysLate = GetDaysOverDue(isbn, loanPeriodDays);
            if (daysLate <= 0)
            {
                return 0;
            }
        ...
        public int GetDaysOverDue(string isbn, int loanPeriodDays)
        {
            Book book = SearchByISBN(isbn);
            if (book != null && book.IsBorrowed && book.BorrowDate.HasValue)
            {
                TimeSpan borrowedDays = DateTime.Now - book.BorrowDate.Value;
                int daysOverdue = borrowedDays.Days - loanPeriodDays;
        ...
        {
            return (GetDaysOverDue(isbn, loanPeriodDays) > 0);          
        }
    }
}
